/**
 * WLOC Giả lập định vị Cloudflare Worker
 * 
 * Trang tĩnh thuần, không cần KV hay bất kỳ liên kết lưu trữ nào
 * Toạ độ được ghi vào $persistentStore cục bộ trên thiết bị thông qua module proxy
 * 
 * Triển khai: Cloudflare Workers (không cần liên kết bất kỳ tài nguyên nào)
 */
...
async function save() {
  ...
  try {
    ...
    if (d.success) {
      ...
      document.getElementById('status').textContent = '✓ Đã ghi: ' + lon.toFixed(6) + ', ' + lat.toFixed(6) + ' · ' + new Date().toLocaleTimeString('zh-CN');
